...
export default class AdHeader extends Component {
    ...
    onHeartPress = async () => {
        if (this.state.heartColor == 'red') {
            await firebase.firestore().collection('Wishlist').where('userID', '==', this.state.uid).where('adID', '==', this.props.adID).get().then(snapshot => {
        ...
        return (
            <View style={{ flex: this.props.flexSize, flexDirection: 'row' }}>
                <View style={{ flex: 8 }}>
                    ...
                    <View style={{ flex: 1 }}>
                        <Text style={this.styles.price}>PKR. {this.state.price}</Text>
                    </View>
