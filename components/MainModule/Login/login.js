...
export default class Login extends Component {
  ...
  emailPasswordLogin = async () => {
    if (!this.isFormEmpty() && this.isErrorFree()) {
      ...
      await login(this.state.email, this.state.password).then(() => {
    ...
    return (
      <View style={{ flex: 1 }}>
        ...
        <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center' }}>
          ...
          <TouchableOpacity onPress={this.googleLogin} style={[styles.btn, styles.loginBtn]}>
            <View style={{ flex: 1, justifyContent: 'center', alignContent: 'center' }}>
              <Image style={styles.icon} source={require('../../../Images/google.png')}></Image>
            </View>
            <View style={{ flex: 5, justifyContent: 'center', alignItems: 'center' }}>
              <Text style={{ color: 'black', fontSize: responsiveFontSize(2) }}>Login with
